...
export const InputPanel = (props: Props) => {
  ...
  if (dataInsightsPipeline.current === null) {
    dataInsightsPipeline.current = new Pipeline([
      new PPLSampleOperator(props.data.search),
      new DataInsightsOperator(props.core.http),
    ]);
  }
  ...
  const onGenerate = useCallback(async () => {
    ...
    for (const insight of selectedInsights) {
      const pipeline = new Pipeline([
        new Text2PPLOperator(props.core.http),
        new PPLSampleOperator(props.data.search),
        new Text2VegaOperator(props.core.http, props.core.savedObjects),
      ]);
      try {
